  ...
  async function ensureFirebaseConfig(){
    ...
    await new Promise((resolve,reject)=>{
      ...
      if(existing){
        ...
        const wait=()=>{
          if(window.YMS_FIREBASE_CONFIG) return resolve();
          if(Date.now()-started>4000) return reject(new Error('Firebase 설정을 불러오지 못했습니다'));
          setTimeout(wait,40);
        };
        wait();
        return;
  ...
  async function relinkForNewUid(oldUid,newUid,role,childIds,linkedStudentId){
    if(role==='PARENT'&&childIds.length){
      await Promise.all(childIds.map(id=>_tFetch(`tables/students/${id}`,{
        method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({parentId:newUid})
      })));
    }
    if(role==='STUDENT'){
      ...
      if(sid){
        const r=await _tFetch(`tables/students/${sid}`,{
          method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({userId:newUid})
        });
        if(!r.ok) throw new Error(`학생 계정 연결 갱신 실패 (HTTP ${r.status})`);
      }
    }
  }
  ...
  window.addEventListener('load',()=>{
    ...
    window.submitAcctForm=async function(e){
      ...
      try{
        ...
        await linkStudent(savedUser,role,name,isEdit||loginChanged);
