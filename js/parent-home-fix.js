...
(function(){
  ...
  // Legacy helpers used by parent-home.html
  window.YMS_UI = window.YMS_UI || {};
  if (!window.YMS_UI.renderEmpty) {
    window.YMS_UI.renderEmpty = function(message){
      return `<div class="empty-state"><div class="empty-icon">📭</div><div class="empty-msg">${String(message||'내용이 없습니다')}</div></div>`;
    };
  }
  ...
  if (!window.renderHomeworkItem) {
    window.renderHomeworkItem=function(hw,onClick){
      ...
      item.type='button'; item.className='parent-list-item';
      ...
      item.addEventListener('click',()=>onClick&&onClick(hw));
      return item;
    };
  }
  ...
  if (!window.renderNoticeItem) {
    window.renderNoticeItem=function(n,onClick){
      ...
      item.addEventListener('click',()=>onClick&&onClick(n));
      return item;
    };
  }
  ...
  css.textContent=`
    body{background:#F4F7FD!important}
    .app-wrapper{max-width:560px!important;margin:0 auto!important;background:#F4F7FD!important;min-height:100vh!important;padding-bottom:86px!important}
    .app-bar{height:68px;display:flex;align-items:center;justify-content:space-between;padding:0 20px;background:#fff;border-bottom:1px solid #E3E8F4;position:sticky;top:0;z-index:50}
    .app-bar-title{font-size:18px;font-weight:900;color:#14245A;letter-spacing:-.4px}.app-bar-right{display:flex;gap:8px}.icon-btn{width:40px;height:40px;border:0;border-radius:12px;background:#EEF3FB;display:grid;place-items:center;font-size:18px;cursor:pointer}
    ...
    .empty-state{padding:24px 12px;text-align:center;color:#8A96B2}.empty-icon{font-size:28px;margin-bottom:6px}.empty-msg{font-size:12px}
    .tab-bar{position:fixed;left:50%;bottom:0;transform:translateX(-50%);width:min(100%,560px);height:70px;padding:7px 10px calc(7px + env(safe-area-inset-bottom));display:grid;grid-template-columns:repeat(4,1fr);background:rgba(255,255,255,.97);border-top:1px solid #E3E8F4;z-index:100;backdrop-filter:blur(12px)}.parent-tab{border:0;background:transparent;color:#8A96B2;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;font-size:20px;font-weight:700}.parent-tab small{font-size:10px}.parent-tab.active{color:#1E3278}
    .modal-overlay{position:fixed;inset:0;background:rgba(20,36,90,.45);z-index:1000;display:flex;align-items:flex-end;justify-content:center}.modal-overlay.hidden{display:none!important}.modal-sheet{width:min(100%,560px);max-height:88vh;overflow:auto;background:#fff;border-radius:24px 24px 0 0;padding:20px}.modal-handle{width:44px;height:4px;background:#D8DFEC;border-radius:99px;margin:0 auto 16px}.modal-title{font-size:18px;font-weight:900;color:#14245A;margin-bottom:16px}.btn{border:0;border-radius:12px;padding:11px 14px;font:inherit;font-weight:750;cursor:pointer}.btn-ghost{background:#F3F6FB;color:#34405F}.btn-full{width:100%}.divider{height:1px;background:#EEF1F7}.mt-12{margin-top:12px}.mt-16{margin-top:16px}.mt-4{margin-top:4px}
    .chip{display:inline-flex;padding:4px 8px;border-radius:999px;font-size:11px;background:#F0F3F8;color:#53617F}.chip-orange{background:#EEF3FB;color:#1E3278}.chip-gray{background:#F2F4F8;color:#65718B}
