...
(function(){
  ...
  if (!window.ymsRenderTabBar) {
    window.ymsRenderTabBar=function(active){
      ...
      const tabs=[['parent-home.html','⌂','홈'],['homework.html','▣','숙제'],['notices.html','●','공지'],['counseling.html','💬','상담']];
  ...
  // Parent accounts may not list every student. Convert the old full-list request
  // into individual reads for only the linked childIds stored on the signed-in profile.
  ...
  css.textContent=`
    ...
    .empty-state{padding:24px 12px;text-align:center;color:#8A96B2}.empty-icon{font-size:28px;margin-bottom:6px}.empty-msg{font-size:12px}.tab-bar{position:fixed;left:50%;bottom:0;transform:translateX(-50%);width:min(100%,560px);height:70px;padding:7px 10px calc(7px + env(safe-area-inset-bottom));display:grid;grid-template-columns:repeat(4,1fr);background:rgba(255,255,255,.97);border-top:1px solid #E3E8F4;z-index:100;backdrop-filter:blur(12px)}.parent-tab{border:0;background:transparent;color:#8A96B2;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;font-size:20px;font-weight:700}.parent-tab small{font-size:10px}.parent-tab.active{color:#1E3278}
