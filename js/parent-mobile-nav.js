/* YMS parent-only mobile bottom navigation */
(function(){
  'use strict';
  ...
  css.textContent=`
    ...
    .tab-bar{position:fixed!important;left:50%!important;bottom:0!important;transform:translateX(-50%)!important;width:min(100%,560px)!important;height:70px!important;padding:7px 8px calc(7px + env(safe-area-inset-bottom))!important;display:grid!important;grid-template-columns:repeat(5,1fr)!important;background:rgba(255,255,255,.98)!important;border-top:1px solid #E3E8F4!important;z-index:1000!important;box-shadow:0 -4px 16px rgba(30,50,120,.06)!important;backdrop-filter:blur(12px)!important}
    .parent-tab{appearance:none!important;border:0!important;background:transparent!important;color:#8A96B2!important;display:flex!important;flex-direction:column!important;align-items:center!important;justify-content:center!important;gap:3px!important;padding:3px 0!important;font:inherit!important;cursor:pointer!important;min-width:0!important}
  ...
  window.ymsRenderTabBar=function(active){
    ...
    bar.innerHTML=tabs.map(([href,icon,label])=>`<button type="button" class="parent-tab ${active===href?'active':''}" onclick="location.href='${href}'"><span>${icon}</span><small>${label}</small></button>`).join('');
  };

  const current=location.pathname.split('/').pop()||'parent-home.html';
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',()=>window.ymsRenderTabBar(current));
  else window.ymsRenderTabBar(current);
})();
