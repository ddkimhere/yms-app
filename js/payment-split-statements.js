...
(function(){
  ...
  async function saveBookFeeJpg(studentId,targetMonth){
    ...
    const fees=allFees.filter(f=>String(f.status||'REGISTERED')!=='CANCELLED'&&String(f.billingMonth||f.month||f.registeredAt||'').slice(0,7)===String(targetMonth)).sort((a,b)=>String(a.registrationDate||a.registeredAt||'').localeCompare(String(b.registrationDate||b.registeredAt||'')));
    ...
    text(ctx,'※ 교육비와 교재비는 별도 내역서로 안내됩니다.',110,1100,28,700,'#526080');text(ctx,'교재비 납부 방법은 학원 안내에 따라 확인해주세요.',110,1150,26,600,'#7A87A8');text(ctx,'YMS 부송관 영어 · 문의 063-832-0219',540,1260,23,650,'#8A96B2','center');
  ...
  function installTuitionSplit(){
    const old=window.downloadTuitionJpgPayload;
    if(typeof old!=='function'||old.__splitTuition) return;
    const f=async function(payload){
      const p={...(payload||{})};
      const tuition=Number(p.tuitionAmount??p.amount??0);
      p.extraAmount=0;
      p.amount=tuition;
      return old(p);
    };
  ...
  function enhanceButtons(){
    installTuitionSplit();
    document.querySelectorAll('.yp-jpg-btn').forEach(btn=>{
      ...
      const book=document.createElement('button');book.type='button';book.className='yp-jpg-btn yp-book-jpg-btn';book.textContent='📚 교재비 JPG';book.onclick=()=>saveBookFeeJpg(m[1],m[2]);btn.after(book);
    });
  }
