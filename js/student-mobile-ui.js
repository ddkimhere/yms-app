...
(function(){
  ...
  if(!document.getElementById('yms-student-mobile-style')){
    ...
    s.textContent=`
      ...
      #tabBar.tab-bar{position:fixed!important;left:50%!important;bottom:0!important;top:auto!important;transform:translateX(-50%)!important;width:min(100%,560px)!important;height:72px!important;padding:7px 10px calc(7px + env(safe-area-inset-bottom))!important;display:grid!important;grid-template-columns:repeat(4,minmax(0,1fr))!important;background:rgba(255,255,255,.98)!important;border:0!important;border-top:1px solid #E3E8F4!important;box-shadow:0 -4px 18px rgba(30,50,120,.07)!important;z-index:99999!important;backdrop-filter:blur(12px)!important}
  ...
  window.ymsRenderTabBar=function(active){
    ...
    const tabs=[['student-home.html','⌂','홈'],['homework.html','▣','숙제'],['notices.html','●','공지'],['counseling.html','💬','상담']];
  ...
  function sharedTop(){
    const page=current();if(!['homework.html','notices.html','counseling.html'].includes(page))return;
    const bar=document.querySelector('.app-bar');if(!bar)return;
    const title={ 'homework.html':'숙제','notices.html':'공지사항','counseling.html':'상담'}[page];
