...
(function(){
  ...
  const pageInfo={
    ...
    'counseling.html':{icon:'👥',title:'학생 관리'},
  ...
  if(!document.getElementById('yms-teacher-mobile-chrome-style')){
    ...
    s.textContent=`
      ...
      @media(max-width:700px){
        ...
        .app-bar-left{display:flex!important;align-items:center!important;gap:12px!important;min-width:0!important;flex:1!important}.app-bar-right{display:flex!important;align-items:center!important;gap:8px!important;flex:0 0 auto!important}.app-bar .icon-btn{border:0!important;background:#EEF3FB!important;width:40px!important;height:40px!important;min-width:40px!important;border-radius:12px!important;font-size:18px!important;color:#14245A!important;display:grid!important;place-items:center!important;padding:0!important;margin:0!important;box-shadow:none!important}.app-bar .yms-teacher-page-icon{font-size:18px!important;line-height:1!important;flex:0 0 auto!important}.app-bar-left>span:not(.yms-teacher-page-icon){font-size:18px!important;font-weight:900!important;color:#14245A!important;letter-spacing:0!important;line-height:1.2!important;white-space:nowrap!important}#roleChip{display:none!important}.filter-scroll,#teacherView{padding-top:18px!important}.app-bar .fab{position:fixed!important}#tabBar.tab-bar{display:none!important}
        #teacherMobileNav{box-sizing:border-box;position:fixed;left:50%;bottom:0;transform:translateX(-50%);width:min(100%,560px);height:calc(82px + env(safe-area-inset-bottom));padding:8px 18px calc(8px + env(safe-area-inset-bottom));display:grid;grid-template-columns:repeat(4,minmax(0,1fr));align-items:stretch;background:#fff;border-top:1px solid #E3E8F4;box-shadow:0 -3px 14px rgba(30,50,120,.05);z-index:99999}#teacherMobileNav .tmn-btn{appearance:none;-webkit-appearance:none;border:0;background:transparent;color:#94A0B8;text-decoration:none;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:5px;padding:4px 0;margin:0;min-width:0;font-family:-apple-system,BlinkMacSystemFont,'Noto Sans KR','Segoe UI',sans-serif;font-weight:800}#teacherMobileNav .tmn-btn .ico{font-size:23px;line-height:1;font-weight:700}#teacherMobileNav .tmn-btn small{font-size:11px;line-height:1;color:inherit;white-space:nowrap;font-weight:850}#teacherMobileNav .tmn-btn.active{color:#1E3278}
      }
    `;
  ...
  function setupPageChrome(){
    if(!pageInfo)return;
    const bar=document.querySelector('.app-bar');const left=bar?.querySelector('.app-bar-left');if(!bar||!left)return;
    document.getElementById('roleChip')?.style.setProperty('display','none');
    const title=[...left.querySelectorAll('span')].find(el=>!el.classList.contains('yms-teacher-page-icon'));if(title)title.textContent=pageInfo.title;
    if(!left.querySelector('.yms-teacher-page-icon')){const icon=document.createElement('span');icon.className='yms-teacher-page-icon';icon.textContent=pageInfo.icon;if(title)left.insertBefore(icon,title);else left.appendChild(icon);}
  }
  function installNav(){let nav=document.getElementById('teacherMobileNav');if(!nav){nav=document.createElement('nav');nav.id='teacherMobileNav';document.body.appendChild(nav);}const active=path==='teacher-home.html'?'teacher-home.html':path;const tabs=[['teacher-home.html','⌂','홈'],['homework.html','▣','숙제'],['notices.html','●','공지'],['counseling.html','👥','학생관리']];nav.innerHTML=tabs.map(([href,icon,label])=>`<a class="tmn-btn ${active===href?'active':''}" href="${href}"><span class="ico">${icon}</span><small>${label}</small></a>`).join('');}
  function setup(){setupPageChrome();installNav();}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',setup,{once:true});else setup();window.addEventListener('load',setup,{once:true});
})();
