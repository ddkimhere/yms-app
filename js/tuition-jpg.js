...
(function(){
  ...
  async function makeJpg(p){
    ...
    round(ctx,95,430,890,320,26,'#F8FAFE','#E3E8F4');
    const base=Number(p.baseAmount??p.amount??0),discount=Number(p.discountAmount||0),final=Number(p.amount||Math.max(0,base-discount));
    text(ctx,'기본 교육비',135,505,30,650,'#65718B'); text(ctx,money(base),935,505,32,750,'#1A2340','right');
    text(ctx,'할인 사유',135,570,30,650,'#65718B'); text(ctx,discount?(p.discountReason||'-'):'없음',935,570,30,650,'#1A2340','right');
    text(ctx,'할인 금액',135,635,30,650,'#65718B'); text(ctx,discount?'- '+money(discount):'0원',935,635,32,750,discount?'#E04040':'#1A2340','right');
    ctx.strokeStyle='#DCE4F3';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(135,675);ctx.lineTo(935,675);ctx.stroke();
    text(ctx,'최종 납부금액',135,725,34,800,'#14245A'); text(ctx,money(final),935,725,46,900,'#1E3278','right');

    const type=guideType(p),acct=type==='CASH'?CASH:OTHER;
    text(ctx,type==='CASH'?'현금결제 안내':'그 외 결제 안내',110,825,36,850,'#14245A');
    round(ctx,95,860,890,type==='CASH'?235:330,26,'#EEF3FB');
    text(ctx,acct.bank,135,930,30,750,'#1E3278');
    text(ctx,acct.account,135,995,42,900,'#14245A');
    text(ctx,`예금주 ${acct.holder}`,135,1050,27,650,'#65718B');

    if(type!=='CASH'){
      try{
        const img=await loadImg(QR_SRC);
        ctx.fillStyle='#FFFFFF';ctx.fillRect(720,890,205,205);ctx.drawImage(img,730,900,185,185);
        text(ctx,'익산 다이로움 결제',822,1120,23,700,'#1E3278','center');
      }catch(e){console.warn('[YMS] QR load failed',e);}
      text(ctx,'계좌이체 또는 다이로움 QR로 결제해 주세요.',135,1145,25,600,'#526080');
    }else{
      text(ctx,'현금결제 대상자는 위 계좌로 입금해 주세요.',135,1115,25,600,'#526080');
    }
  ...
  window.downloadTuitionJpg=async function(id){
    try{
      ...
      const p=await getPayment(id),url=await makeJpg(p),a=document.createElement('a');
