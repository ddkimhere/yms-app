...
if(url.pathname.endsWith('/homework.html')){pre=['js/homework-ui-compat.js','js/homework-personal.js','js/homework-filter.js','js/parent-tabbar.js','js/parent-topbar.js','js/student-mobile-ui.js','js/teacher-mobile-nav.js','js/teacher-mobile-ui.js'];end=['js/homework-completion.js','js/homework-delete-fix.js'];}
...
if(pm.length)html=html.includes(appTag)?html.replace(appTag,appTag+pm.map(s=>`<script src="${s}?v=3.86.0"></script>`).join('')):html.replace('</body>',pm.map(s=>`<script src="${s}?v=3.86.0"></script>`).join('')+'</body>');
const em=end.filter(s=>!html.includes(s));
if(em.length)html=html.replace('</body>',em.map(s=>`<script src="${s}?v=3.86.0"></script>`).join('')+'</body>');
