/* YMS Master Track — Service Worker v3.64.0 */
const CACHE_NAME='yms-v3.64.0';
...
if(url.pathname.endsWith('/teacher-home.html')){
  end=['js/admin-multirole-fix.js','js/teacher-mobile-nav.js','js/teacher-homework-form-fix.js','js/teacher-book-fee.js','js/class-grade-sort.js','js/home-notices.js','js/home-widgets.js'];
...
if(url.pathname.endsWith('/parent-home.html')){pre=['js/parent-home-fix.js','js/notice-role.js','js/home-notices.js','js/parent-tabbar.js'];end=['js/home-widgets.js'];}
if(url.pathname.endsWith('/parent-payment.html'))pre=['js/parent-payment-data.js','js/parent-tabbar.js','js/parent-topbar.js'];
if(url.pathname.endsWith('/student-home.html')){pre=['js/notice-role.js','js/home-notices.js','js/student-mobile-ui.js'];end=['js/home-widgets.js'];}
...
if(pm.length)html=html.includes(appTag)?html.replace(appTag,appTag+pm.map(s=>`<script src="${s}?v=3.64.0"></script>`).join('')):html.replace('</body>',pm.map(s=>`<script src="${s}?v=3.64.0"></script>`).join('')+'</body>');
const em=end.filter(s=>!html.includes(s));
if(em.length)html=html.replace('</body>',em.map(s=>`<script src="${s}?v=3.64.0"></script>`).join('')+'</body>');
