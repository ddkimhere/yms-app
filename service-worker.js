/* YMS Master Track — Service Worker v3.74.0 */
const CACHE_NAME='yms-v3.74.0';
...
if(pm.length)html=html.includes(appTag)?html.replace(appTag,appTag+pm.map(s=>`<script src="${s}?v=3.74.0"></script>`).join('')):html.replace('</body>',pm.map(s=>`<script src="${s}?v=3.74.0"></script>`).join('')+'</body>');
const em=end.filter(s=>!html.includes(s));
if(em.length)html=html.replace('</body>',em.map(s=>`<script src="${s}?v=3.74.0"></script>`).join('')+'</body>');
