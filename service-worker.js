/* YMS Master Track — Service Worker v3.87.0 */
const CACHE_NAME='yms-v3.87.0';
...
if(url.pathname.endsWith('/teacher-home.html')){
  end=['js/teacher-mobile-nav.js','js/teacher-homework-form-fix.js','js/teacher-home-actions-fix.js','js/teacher-home-scope-lite.js','js/teacher-book-fee.js','js/home-notices.js','js/home-widgets.js','js/homework-completion.js'];
...
if(url.pathname.endsWith('/attendance.html')){pre=['js/attendance-teacher-mode.js','js/teacher-mobile-nav.js','js/teacher-mobile-ui.js','js/teacher-roster-fix.js'];end=['js/admin-attendance-all.js'];}
if(url.pathname.endsWith('/homework.html')){
  pre=['js/homework-ui-compat.js','js/parent-tabbar.js','js/parent-topbar.js','js/student-mobile-ui.js','js/teacher-mobile-nav.js','js/teacher-mobile-ui.js'];
  end=['js/homework-personal.js','js/homework-filter.js','js/homework-completion.js','js/homework-delete-fix.js'];
}
if(url.pathname.endsWith('/calendar.html'))pre=['js/parent-tabbar.js','js/student-mobile-ui.js','js/teacher-mobile-nav.js','js/teacher-mobile-ui.js','js/calendar-live.js'];
...
const isParentPage=['/homework.html','/notices.html','/counseling.html','/parent-home.html','/parent-payment.html','/calendar.html'].some(p=>url.pathname.endsWith(p));
...
if(pm.length)html=html.includes(appTag)?html.replace(appTag,appTag+pm.map(s=>`<script src="${s}?v=3.87.0"></script>`).join('')):html.replace('</body>',pm.map(s=>`<script src="${s}?v=3.87.0"></script>`).join('')+'</body>');
const em=end.filter(s=>!html.includes(s));
if(em.length)html=html.replace('</body>',em.map(s=>`<script src="${s}?v=3.87.0"></script>`).join('')+'</body>');
