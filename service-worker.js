...
if(url.pathname.endsWith('/homework.html'))pre=['js/parent-tabbar.js','js/parent-topbar.js','js/student-mobile-ui.js'];
if(url.pathname.endsWith('/notices.html'))pre=['js/notice-role.js','js/parent-tabbar.js','js/parent-topbar.js','js/student-mobile-ui.js'];
if(url.pathname.endsWith('/counseling.html'))pre=['js/parent-tabbar.js','js/parent-topbar.js','js/student-mobile-ui.js'];
...
if(pm.length)html=html.includes(appTag)?html.replace(appTag,appTag+pm.map(s=>`<script src="${s}?v=3.28.0"></script>`).join('')):html.replace('</body>',pm.map(s=>`<script src="${s}?v=3.28.0"></script>`).join('')+'</body>');
const em=end.filter(s=>!html.includes(s));
if(em.length)html=html.replace('</body>',em.map(s=>`<script src="${s}?v=3.28.0"></script>`).join('')+'</body>');
