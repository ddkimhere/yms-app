/* YMS Master Track — Service Worker v4.09.0 */
const CACHE_NAME='yms-v4.09.0';
...
async function patchHtml(req,res){
  try{
    ...
    const preTags=pre.filter(s=>!html.includes(s)).map(s=>`<script src="${s}?v=4.09.0"></script>`).join('');
    if(preTags)html=html.includes(appTag)?html.replace(appTag,appTag+preTags):html.replace('</body>',preTags+'</body>');
    const endTags=end.filter(s=>!html.includes(s)).map(s=>`<script src="${s}?v=4.09.0"></script>`).join('');
