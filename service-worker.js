/* YMS Master Track — Service Worker v3.79.0 */
const CACHE_NAME='yms-v3.79.0';
...
if(url.pathname.endsWith('/teacher-home.html')){
  end=['js/teacher-mobile-nav.js','js/teacher-homework-form-fix.js','js/teacher-home-actions-fix.js','js/teacher-home-scope-lite.js','js/teacher-book-fee.js','js/class-grade-sort.js','js/home-notices.js','js/home-widgets.js'];
...
if(pm.length)html=html.includes(appTag)?html.replace(appTag,appTag+pm.map(s=>`<script src="${s}?v=3.79.0"></script>`).join('')):html.replace('</body>',pm.map(s=>`<script src="${s}?v=3.79.0"></script>`).join('')+'</body>');
const em=end.filter(s=>!html.includes(s));
if(em.length)html=html.replace('</body>',em.map(s=>`<script src="${s}?v=3.79.0"></script>`).join('')+'</body>');
