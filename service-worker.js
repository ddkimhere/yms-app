...
if(url.pathname.endsWith('/teacher-home.html'))end=['js/admin-multirole-fix.js','js/teacher-mobile-nav.js'];
...
if(pm.length)html=html.includes(appTag)?html.replace(appTag,appTag+pm.map(s=>`<script src="${s}?v=3.29.0"></script>`).join('')):html.replace('</body>',pm.map(s=>`<script src="${s}?v=3.29.0"></script>`).join('')+'</body>');
const em=end.filter(s=>!html.includes(s));
if(em.length)html=html.replace('</body>',em.map(s=>`<script src="${s}?v=3.29.0"></script>`).join('')+'</body>');
